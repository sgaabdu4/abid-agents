...
assertIncludes(routeMapText, 'vendor/skill-upstreams/lavish-axi/skills/lavish/SKILL.md');
...
assertIncludes(featureFlowHtml, 'Lavish helps UI choices when text is not enough');
...
assertIncludes(featureFlowHtml, 'Use Lavish inside <code>/grill-me</code>.');
...
assertIncludes(fs.readFileSync(path.join(repo, '.gitmodules'), 'utf8'), 'vendor/skill-upstreams/lavish-axi');
...
assertIncludes(grillUiFlowText, 'low-fi Lavish wireflow first');
assertIncludes(grillUiFlowText, 'wireflows/maps, not visual direction');
assertIncludes(grillUiFlowText, 'vendor/skill-upstreams/lavish-axi/skills/lavish/SKILL.md');
assertIncludes(grillVisualDesignText, 'Lavish direction boards');
assertIncludes(grillVisualDesignText, 'subject-project tokens/components/CSS vars');
assertIncludes(grillVisualDesignText, 'Fix `layout_warnings`');
...
assertIncludes(fs.readFileSync(path.join(repo, 'scripts', 'update-submodules.sh'), 'utf8'), 'vendor/skill-upstreams/lavish-axi:skills/lavish', 'submodule updater must keep Lavish sparse checkout on the vendored skill');
