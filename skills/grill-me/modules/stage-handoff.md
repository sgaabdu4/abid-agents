...
Do not create this while interviewing unless the user asks for docs/status. Use compact form at stage close. Expand only while writing final plan. At stage close, refine the answer ledger into this summary after the clarity gate passes; ask one clarification at a time if refinement finds a blocker, repeating until resolved or explicitly parked by the user.
